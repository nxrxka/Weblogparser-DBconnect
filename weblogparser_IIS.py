...
def create_database():
    ...
    cursor = conn.cursor()
    sql = f"CREATE TABLE Result(Local_IP text, Client_IP text, Date text, Method text, URL text, Argument text, Port text, Status_Code text, Size text, User_Agent text, Refer text, File_Name text)"
    cursor.execute(sql)
    return cursor, conn
...
def log_parse(contents, input_val, cursor):
    try:
        ...
        return
    sql = f"INSERT INTO Result(Local_IP, Client_IP, Date, Method, URL, Argument, Port, Status_Code, Size, User_Agent, Refer, File_Name) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
    val = (
...
def main():
    input_val = "/Users/-userID-/-weblogtypes-/type1.log"
